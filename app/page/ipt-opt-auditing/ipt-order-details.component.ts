...
@Component({
    ...
    styleUrls: [ 'ipt-order-details.component.css', '../common/reset.css' ],
    providers: [ IptOrderAuditService, IptOrderDetailsService ]
})
...
export class IptOrderDetailsComponent implements OnInit {
    ...
    getPatientInfo(orderId: number): void {
         this.iptOrderDetailsService.getPatientInfo(orderId).then(result => {
             ...
             this.totalDays = Math.ceil(((new Date().getTime()) - (this.patientInfo.hospitalizedTime || this.defaultDate.getTime()) ) / 1000 / 60 / 60 / 24);
             this.getSummaryMonthList();
             this.getSummaryDateList(new Date());
         });
    }
    getSummaryMonthList():void {
        let startTime = this.patientInfo.hospitalizedTime || this.defaultDate.getTime();
        ...
        this.iptOrderDetailsService.getIptOrderList(orderId, startDate, period).then(resultList => {
            ...
            if(startDate != '' && period != 0){
                ...
                for(let groupNum in resultList){
                    ...
                    this.getEngineMsgList(this.iptOrderInput.recipeId, groupNum, orderIdList, hisOrderIdList, null, null);
                }
                this.checkedRecipe =  this.iptOrderList[0];
            }

            //this.orderApiComplete++
        });
    }
    getHerbIptOrderList(orderId: number, startDate: string, period: number): void{
        this.iptOrderDetailsService.getHerbIptOrderList(orderId, startDate, period).then(resultList => {
            ...
            if(startDate != '' && period != 0){
                ...
                for(let groupNum in resultList){
                    ...
                    for(let order of resultListArr){
                        if(!(order.orderValidTime > (invalidDate.getTime() + 86400000) || (order.orderInvalidTime ? (order.orderInvalidTime < validDate.getTime()) : false))){
                            // for(let inOrder of order.itemList){
                            //     order.drugName = inOrder.drugName;
                            // }
                ...
                for(let groupNum in resultList){
                    ...
                    resultListArr.push(resultList[groupNum]);
                    let group = {
                        'groupNum': resultList[groupNum].id,
                    ...
                    this.iptOrderList.push(group);
                    this.getEngineMsgList(this.iptOrderInput.recipeId, group.groupNum, null, null, herbOrderIdList, hisHerbOrderIdList);
                }
                this.checkedRecipe =  this.iptOrderList[0];
            }
    ...
    //对药嘱排序，可以被操作的排在前面
    checkedHoldOrder(orders: any[]){
        Array.prototype.sort.call(orders, (a, b) => {
            let v1 = a.status == -1 ? (a.passStatus != 1 && a.passStatus != 2 ? 1000 : 1) : a.status;
            let v2 = b.status == -1 ? (b.passStatus != 1 && b.passStatus != 2 ? 1000 : 1) : b.status;
        ...
        for(let i = 0; i < orders.length; i++ ){
            ...
            if(orders[i].status == -1){
                this.checkedRecipe = this.iptOrderList[i];
                return;
            }
            //有挂起的医嘱二次操作
    ...
    changeCheckedOrder(order: any): void {
        this.auditInfoTxt = "审核意见...";
        this.checkedRecipe = order;
    }
    engineMsgChecked(drugMsg: any, level: number, engineMsgGroup: any){
        if(engineMsgGroup.isChecked || drugMsg.msg.notReject) return;    //已经被通过或打回
        
        if(!(drugMsg.resultMsg.operateStatus == 1 || drugMsg.resultMsg.operateStatus == 2)){
            drugMsg.resultMsg.operateStatus = 2;
            if(level == 1){
                engineMsgGroup.checkedNum++;
            }
        }else{
            drugMsg.resultMsg.operateStatus = null;
            if(level == 1){
                engineMsgGroup.checkedNum--;
            }
        }
    }
    engineMsgDblChecked(drugMsg: any, engineMsgGroup: any){
        if(engineMsgGroup.isChecked || drugMsg.msg.notReject) return;    //已经被通过或打回
        if(drugMsg.resultMsg.operateStatus != 2){
            drugMsg.resultMsg.operateStatus = 2;
        }else{
            drugMsg.resultMsg.operateStatus = 1;
        }
    }
    getEngineMsgList(id: string, groupNo: string, orderIdList: any, hisOrderIdList:any, herbOrderIdList: any, hisHerbOrderIdList:any):void {
        this.iptOrderDetailsService.getEngineMsgList(id, groupNo, orderIdList, hisOrderIdList, herbOrderIdList, hisHerbOrderIdList).then(result => {
            ...
            if(result && result.hasOwnProperty('drugMsgMap')){
                for(let iptOrder of this.iptOrderList){
                    if(iptOrder.groupNum == groupNo){
                        ...
                        iptOrder.orderType = orderType;
                        //已经被操作过，判断为已操作
                        if(iptOrder.auditHandle.notReject == 1 || this.canApply(iptOrder) == 1){
                            //iptOrder.isChecked = true;
                            iptOrder.passStatus = 1;//前端参数，控制排序
                        }
                        if(iptOrder.auditHandle.notReject == 2 || this.canApply(iptOrder) == 2){
                            //iptOrder.isChecked = true;
                            iptOrder.passStatus = 2;//前端参数，控制排序
                        }

                        if(iptOrder.auditHandle.auditStatus == 1){
                            iptOrder.status = 1;
                            iptOrder.isChecked = true;
                        }else if(iptOrder.auditHandle.auditStatus == 0){
                            iptOrder.status = 0;
                            iptOrder.isChecked = true;
                        }else if(iptOrder.auditHandle.auditStatus == 4){
                            iptOrder.status = 4;
                        }
                    }
                }
                ...
                for(let iptOrder of this.iptOrderList){
                    if(iptOrder.groupNum == groupNo){
                        iptOrder.auditHandle = null;
                    }
                }
            }
            //选中被挂起的药嘱
            this.checkedHoldOrder(this.iptOrderList);
            //this.flag ++;
        });
    }
    saveEngineMsgList(engineMsgGroup: any, status: number, info: string): void {
        if(status == 0 && (info == '' || info == '审核意见...') && engineMsgGroup.checkedNum <= 0){
            this.promptComponent.alert('请选择警示信息或添加审核意见。')
            return;
        }
        
        if(engineMsgGroup.passStatus == 1 || engineMsgGroup.passStatus == 2) return;

        if(engineMsgGroup.engineMsgs && engineMsgGroup.engineMsgs.length > 0){
            for(let engineMsgs of engineMsgGroup.engineMsgs){
                ...
                let needNewOne = true;
                for(let groupOrder of this.groupOrderList) {
                    if(groupOrder.groupNo == engineMsgGroup.groupNum) {
                        groupOrder.auditBoList = groupOrder.auditBoList.concat(auditBoList);
                        needNewOne = false;
                    }
                }
                if(needNewOne) {
                    this.groupOrderList.push({
                        "auditBoList": auditBoList,
                        "groupNo": engineMsgGroup.groupNum,
                        "auditInfo": info == '审核意见...' ? '' : info,
                        "auditStatus": status,
                        "engineId": this.iptOrderInput.recipeId,
                        "orderType": engineMsgGroup.orderType
                    });
                }
            }
        } else {
        ...
        let postData = {
            'groupOrderList':this.groupOrderList
        }
        ...
        for(let order of this.iptOrderList){
            if(!(order.isChecked || order.passStatus == 1 || order.passStatus == 2)){
        ...
        if(this.isAllRecipeChecked){
           ...
           // this.checkedRecipe = 
           let i = this.iptOrderList.indexOf(this.checkedRecipe);
           if(this.iptOrderList[i + 1] && !this.iptOrderList[i + 1].isChecked){
               this.checkedRecipe = this.iptOrderList[i + 1];
               this.auditInfoTxt = "审核意见...";
           }
        }
    }
    changeOrder(order: any, validDate: Date, invalidDate: Date, arrayNo: number): void{
        ...
        // this.summaryList.orderList.orderOptionList[order.drugName || '护理医嘱'].push({
        ...
        this.iptOrderDetailsService.getVitalSignList(orderId, startDate, period).then(result => {
            ...
            if(!isPopup){
                ...
                for(let vitalSignOption of this.summaryList.vitalSignList.vitalSignOptionList){
                    this.getPositionArr(vitalSignOption, this.summaryList.vitalSignList.vitalSignWidth - (vitalSignOption.leftPos + vitalSignOption.rightPos) * this.summaryTDWidth, this.summaryList.vitalSignList.vitalSignHeight);
                }
            } else {
    ...
    vitalSignShow(): void {
        ...
        if(vitalSignDom){
            ...
            for(let vitalSignOption of this.vitalSignOptionList){
                this.getPositionArr(vitalSignOption, this.vitalSignWidth - (vitalSignOption.leftPos + vitalSignOption.rightPos) * document.getElementById("vitalSignTDList").offsetWidth, this.vitalSignHeight);
            }
        }
    }
    vitalSignHide(): void {
    ...
    getSummaryData(startDate: string, period: number): void {
        this.getVitalSignList(this.iptOrderInput.recipeId, startDate, period, false);
        ...
        this.getOrderList(this.iptOrderInput.recipeId, startDate, period);
    ...
    getNextRecipe(): void {
        this.iptOrderDetailsService.getNextRecipe(this.iptOrderInput.recipeId).then(result => {
            if(result.code == '200' && result.data){
                this.auditInfoTxt = "审核意见...";
                this.groupOrderList = [];
                this.router.navigate(['/page/ipt-order-details/' + result.data]);
            } else {
                this.backToList();
            }
        });
    ...
    ngOnInit(){
        ...
        this.beatingInterval = setInterval(() => {
            ...
            this.auditingStatus(this.iptOrderInput.recipeId);
        }, 2000);
        
        this.activatedRoute.params.subscribe(iptOrderInput => {
            this.iptOrderInput = iptOrderInput;

            this.getVitalStartDate(this.iptOrderInput.recipeId);
            ...
            this.getOrderList(this.iptOrderInput.recipeId, '', 0);
            ...
            //this.getVitalSignList(this.iptOrderInput.recipeId, this.defaultDate.getTime().toString(), 7, true);
    ...
    //心跳
    // auditBeatingStatus() {
    //     this.iptOrderDetailsService.auditBeatingStatus()
    //         .then(res => {

    //         });
    // }
    auditingStatus(trow: any) {
        this.iptOrderAuditService.auditing(trow)
            .then(res => {
     ...
     * 快速回复
     */
    private auditInfoTxt: string;
    replySelect($event: any){
        this.auditInfoTxt = $event;
    }
    ...
    getVitalStartDate(id: string){
        this.iptOrderDetailsService.getVitalStartDate(id)
            .then(res => {
                if(res.code == '200' && res.data){
                    this.vitalSignStartDate = res.data;
                    this.getVitalSignList(this.iptOrderInput.recipeId, res.data, 7, true);
                }else{
                    this.getVitalSignList(this.iptOrderInput.recipeId, this.defaultDate.getTime().toString(), 7, true);
                }
            })
    ...
    goToDrugDetail(){
        this.iptOrderDetailsService.getDrugApi()
            .then(res => {
                if(res.code == '200' && res.data){
     ...
     * 全部通过
     */
    allPass(isNoReject?: boolean){
        if(isNoReject){
            for(let iptOrder of this.iptOrderList){
                iptOrder.passStatus = undefined;
            }
            for(let iptOrder of this.iptOrderList){
                //对未被操作过的警示信息执行通过操作
                if(iptOrder.status == -1 && !iptOrder.isChecked){
                    this.saveEngineMsgList(iptOrder, 1, '');
                }
            }
        }else{
            for(let iptOrder of this.iptOrderList){
     ...
     * 当前药嘱是否可以被操作
     */
    canApply(engineMsgGroup: any){
        if(engineMsgGroup.engineMsgs && engineMsgGroup.engineMsgs.length <= 0){
            if(engineMsgGroup.auditHandle && engineMsgGroup.auditHandle.notReject)
                return engineMsgGroup.auditHandle.notReject;
            return false;
        }
        
        let engineMsglist = [];
        for(let enginMsg of engineMsgGroup.engineMsgs){
            for(let drugMsg of enginMsg.drugMsg){
                engineMsglist.push(drugMsg.msg)
            }
        }
        for(let msg of engineMsglist){
            if(msg.notReject)
                return msg.notReject;
        }
        return false;
    }
    /**
     ...
     * 临时处理方法
     */
    // private isAllLoad: boolean = false;
    // private orderApiComplete: number = 0;
    // private drugCount: number = 0;
    // private flag: number = 0;
    // cantReject(){
    //     let that = this;
    //     let t = setInterval(() => {
    //         if(that.orderApiComplete == 2 && that.flag == that.drugCount){
    //             clearInterval(t);
    //             for(let iptOrder of that.iptOrderList){
    //                 if(!(iptOrder.passStatus == 1 || iptOrder.passStatus == 2))
    //                     return false;
    //             }
    //             //
    //             that.orderApiComplete = 0;
    //             that.drugCount = 0;
    //             that.flag = 0;
    //             //that.getNextRecipe();
    //             this.allPass(true);
    ...
    getDrugRelated(drugOrder: any){
        ...
        this.noInfo = false;
        this.iptOrderDetailsService.getDrugRelated(this.iptOrderInput.recipeId, drugOrder.zoneId, drugOrder.drugId, dateTemp, 7)
            .then(res => {
                if(res.code == '200'){
                    if(res.data){
                        this.noInfo = true;
