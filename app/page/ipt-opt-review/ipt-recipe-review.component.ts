...
export class IptRecipeReviewComponent implements OnInit {
    ...
    reloadHistory(){
        if(!sessionStorage.getItem('onCurrentPage') && !sessionStorage.getItem('onCurrentPage')){
            this.search();
            return;
        };

        // let pagination:any = sessionStorage.getItem('onCurrentPage');
        // //this.searchParams = sessionStorage.getItem('onCurrentPage');
        // pagination = pagination ? JSON.parse(pagination) : {};
        // //设置table组件的当前分页信息
        // if(pagination.currentPage) this.recipeListTable.currentPage = pagination.currentPage;
        // if(pagination.pageSize) this.recipeListTable.pageSize = pagination.pageSize;

        let historyParam = sessionStorage.getItem('iptRecipeReviewSearchParams');
        if(historyParam){
            ...
            sessionStorage.removeItem('iptRecipeReviewSearchParams');
        }
        ...
        // let tempUrl = this.formatUrl(this.recipeListTable.url, pagination.currentPage, pagination.pageSize);
    ...
    search() {
        ...
        sessionStorage.setItem('onCurrentPage', JSON.stringify(this.searchParams));
    ...
    recipeListTable: any = {
        ...
        pageSize: 20,
        url: "/api/v1/ipt/all/iptList?pageSize={pageSize}&page={currentPage}",
    ...
    getPagination($event: any) {
        ...
        // storage.setItem("currentPage", $event.currentPage);
        // console.log(storage["currentPage"]);
        // sessionStorage.setItem('onCurrentPage', JSON.stringify($event));
        // console.log(sessionStorage.setItem('onCurrentPage', JSON.stringify($event)))
    ...
    goDetail(trow){
        let _param = {
            name: 'iptRecipeReviewSearchParams',
            router: '/page/ipt-recipe-review',
            searchParam:{
        ...
        sessionStorage.setItem('iptRecipeReviewDetails', JSON.stringify(_param));
        this.router.navigate(['/page/ipt-recipe-review-details', trow.engineId]);
    }
}